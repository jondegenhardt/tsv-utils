...
else
{
    int main(string[] cmdArgs)
    {
        ...
        try
        {
            ...
            {
                /* streamSamplingViaSkips is faster with low sample rates. Only used
                 * when using an unpredictable seed. This is to maintain consistency
                 * across multiple runs when the same seed and different sampling rates.
                 */
                enum skipSamplingBreakEvenRate = 0.05;  // Must be < 1.0
                if (cmdopt.sampleRate > 0.0 &&
                    cmdopt.sampleRate <= skipSamplingBreakEvenRate &&
                    !cmdopt.printRandom &&
                    cmdopt.usingUnpredictableSeed)
                {
                    streamSamplingViaSkips(cmdopt, bufferedOutput);
                }
                else
                {
...
struct TsvSampleOptions
{
    ...
    bool usingUnpredictableSeed = false; // Derived. True if --static-seed, --seed-value are not specified.
    ...
    auto processArgs(ref string[] cmdArgs)
    {
        ...
        try
        {
            ...
            if (seedValueOptionArg != 0)
            {
                seed = seedValueOptionArg;
                usingUnpredictableSeed = false;
            }
            else if (staticSeed)
            {
                seed = 2438424139;
 ...
 * is a assigned a random value and output if less than the sampling rate. See also
 * streamSamplingViaSkips, which is faster when the sampling rate is less than 4-5%.
 */
...
/* streamSamplingViaSkips is an alternate implementation of streamSampling. It works by
 * skipping a random number of lines between selections. This is faster when the sampling
 * rate is low, as the greatly reduces the number of calls to the random number generator.
 *
 * Main Result: Performance tests after implementation did not show enough difference to
 * be worth introducing this functionality. As standalone, just simply generating the
 * random numbers, the percentage delta was larger. But in the app context, the percentage
 * delta was small.
 *
 * Notes:
 * - cmdopt.sampleRate (--r|rate) must be strictly between 0.0 and 1.0.
 * - Generating random numbers for selected lines (--p|print-random) is not supported.
 *   Could use the random number used to generate the skip distance, but this is not
 *   consistent with performance enhancement goal.
 * - The lines selected are different than generated by streamSampling even when using
 *   the same random seed.
 */
void streamSamplingViaSkips(OutputRange)(TsvSampleOptions cmdopt, OutputRange outputStream)
    if (isOutputRange!(OutputRange, char))
{
    import std.conv : to;
    ...
    assert(cmdopt.sampleRate > 0.0 && cmdopt.sampleRate < 1.0);

    auto randomGenerator = Random(cmdopt.seed);

    double discardRate = 1.0 - cmdopt.sampleRate;
    double logDiscardRate = log(discardRate);
